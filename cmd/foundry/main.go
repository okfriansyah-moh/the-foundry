...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: foundry <doctor|keygen|login|plan|projection|status|principal|profile|policy|migrate|evidence|cost|budget|audit|mission|intake|product|promotions|opportunity|mockup|bench|agents|skills>")
		os.Exit(1)
	}
