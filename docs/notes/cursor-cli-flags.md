# Cursor CLI flags snapshot (docs/PLAN.md Task 88 / PRV-05)

Dated snapshot of the Cursor CLI invocation this adapter relies on. Mirrors
...
verification path; adjust `headlessArgs`/`defaultBinary` in `adapter.go` if
the real CLI differs).
...
- `cursor-agent` — Cursor's headless agent CLI binary.
- `--print` — non-interactive: emit result to stdout and exit.
- Prompt fed on **stdin** (never argv), per Task 17's precedent.

## Environment allowlist

- `PATH`, `HOME` — tool resolution and config discovery.
- `CURSOR_API_KEY` — provider auth (the deliberately-passed secret).
...
Cursor-specific field names leak into `Summary`. Task 13's verifier decides
