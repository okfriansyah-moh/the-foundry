...
// TestIntegration_HelloWorld runs the real CLI. Gated behind
// RUN_REAL_EXECUTOR=1 (docs/PLAN.md Task 88 Validation) — costs real API
// usage and requires the CLI authenticated in the environment.
func TestIntegration_HelloWorld(t *testing.T) {
	if os.Getenv("RUN_REAL_EXECUTOR") != "1" {
		t.Skip("skipping: set RUN_REAL_EXECUTOR=1 to run the real copilot CLI (costs real API usage)")
	}
	...
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "README.md"), []byte("fixture repo for Task 88 integration test\n"), 0o644); err != nil {
	...
	t.Logf("copilot integration Summary: %+v", summary)
}
