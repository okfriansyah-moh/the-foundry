...
// TestIntegration_HelloWorld runs the real `gemini` CLI. Gated behind
// RUN_REAL_EXECUTOR=1 (docs/PLAN.md Task 87 Validation) — costs real API
// usage and requires `gemini` authenticated in the environment.
func TestIntegration_HelloWorld(t *testing.T) {
	if os.Getenv("RUN_REAL_EXECUTOR") != "1" {
		t.Skip("skipping: set RUN_REAL_EXECUTOR=1 to run the real gemini CLI (costs real API usage)")
	}
	...
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "README.md"), []byte("fixture repo for Task 87 integration test\n"), 0o644); err != nil {
	...
	t.Logf("gemini-cli integration Summary: %+v", summary)
}
